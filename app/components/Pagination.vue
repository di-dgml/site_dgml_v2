...
const paginationButtons = computed(() => {
  ...
  const endPage = Math.min(props.totalPages, startPage + maxButtons - 1)
...
<template>
  ...
  >
    ...
    <div class="flex justify-center items-center gap-2">
      ...
      <span
        v-if="paginationButtons[0] > 2"
        class="text-gray-500 dark:text-gray-400 px-2"
      >...</span>
      ...
      <!-- Last Page -->
      <span
        v-if="paginationButtons[paginationButtons.length - 1] < totalPages - 1"
        class="text-gray-500 dark:text-gray-400 px-2"
      >...</span>
      <button
